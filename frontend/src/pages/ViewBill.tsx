...
const ViewBill: React.FC = () => {
    ...
    // ─── Load templates with saved overrides from localStorage ───────────────
    // Template Library saves settings/fields to localStorage under key
    // 'billsoft_template_overrides'. We merge them here so Bills shows the
    // exact same design the user configured in Template Library.
    const getTemplatesWithOverrides = (): InvoiceTemplate[] => {
        try {
    ...
    // Default to the "isDefault" template from merged list, or first one
    const defaultTemplate = mergedTemplates.find(t => t.isDefault) || mergedTemplates[0];
    ...
    useEffect(() => {
        if (bill) {
            const found = mergedTemplates.find(t => t.id === bill.templateId) || mergedTemplates.find(t => t.isDefault) || mergedTemplates[0];
            setSelectedTemplate(found);
        }
    }, [bill, mergedTemplates]);
    ...
    const handleEmailShare = async () => {
        ...
        try {
            ...
            if (res.ok) {
                ...
                if (data.previewUrl) {
                    // Open the test Ethereal message to view email format instantly
                ...
                >
                    <Box
                        ...
                        sx={{
                            ...
                            minWidth: '210mm',
                            height: 'auto',
                            mb: isMobile ? -20 : 0, // Compensate for scaled height if needed, though 'auto' helps
                            '@media print': {
                                width: '210mm',
                                height: '297mm',
                                overflow: 'hidden',
