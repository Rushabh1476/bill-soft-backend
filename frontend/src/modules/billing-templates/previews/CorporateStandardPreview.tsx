        ...
        {isVisible("5") && (
          <TableContainer component={Box} sx={{ mb: 4, border: `1px solid ${colorScheme}30` }}>
            <Table sx={{ '& th': { backgroundColor: `${colorScheme}15`, fontWeight: 'bold', color: colorScheme } }}>
              <TableHead>
                <TableRow>
                  <TableCell>{getLabel("5", "Description")}</TableCell>
              ...
              <TableBody>
                {invoiceData.items.map((item: any, index: number) => (
                  <TableRow key={index} sx={{ '&:nth-of-type(even)': { backgroundColor: 'grey.50' } }}>
                    ...
                    <TableCell align="right" sx={{ fontWeight: 'bold' }}>₹{item.amount.toLocaleString('en-IN')}</TableCell>
                  </TableRow>
                ))}
              </TableBody>
            </Table>
          ...
          <Box sx={{ p: 2, width: '300px', border: `1px solid ${colorScheme}30`, borderRadius: '4px' }}>
            ...
            {showTaxBreakdown ? (
              (() => {
                ...
                return entries.map(([rate, amt]) => (
                  <Box key={rate} sx={{ display: 'flex', justifyContent: 'space-between', mb: 0.5 }}>
                    ...
                    <Typography variant="body2" sx={{ fontWeight: 'medium' }}>₹{Math.round(amt).toLocaleString('en-IN')}</Typography>
                  </Box>
                ));
              })()
            ) : (
              <Box sx={{ display: 'flex', justifyContent: 'space-between', mb: 2 }}>
                ...
                <Typography variant="body2" fontWeight="bold">₹{invoiceData.tax.toLocaleString('en-IN')}</Typography>
              </Box>
            )}
            ...
            {isVisible("6") && (
              <Box sx={{ display: 'flex', justifyContent: 'space-between', p: 1.5, backgroundColor: colorScheme, color: 'white', borderRadius: 1, mt: 1 }}>
                ...
                <Typography variant="subtitle1" fontWeight="bold">₹{invoiceData.total.toLocaleString('en-IN')}</Typography>
              </Box>
            )}
          </Box>
        </Box>
